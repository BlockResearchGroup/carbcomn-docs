...
The **Thrust Network Analysis (TNA)** method[^4], described in detail in the [next section](thrust_network_analysis.md), provides the computational framework for solving this two-dimensional form-finding problem. TNA is the first computational stage of the CARBCOMN pipeline.
...
**Striatus 2.0: Phoenix** (2024) is a permanent successor to Striatus, improving on the original design in several aspects related to circularity, manufacturing precision, and assembly. The project documents the improved integrated design, engineering, and fabrication framework and provides a detailed comparison between the two iterations, further demonstrating the maturity of the URCM approach for real structural applications[^3].
